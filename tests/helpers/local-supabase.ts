...
export async function seedLocalDatabase() {
  ...
  await admin.from("campaigns").insert({ id: ids.campaign, owner_id: owner.id, name: "E2E 캠페인" }); await admin.from("campaign_forms").insert({ id: ids.form, campaign_id: ids.campaign, template_id: ids.template, public_id: ids.publicId, title: "자료 신청", description: "이메일을 입력하세요", submit_label: "자료 받기" })
  return { admin }
}
