...
## 개선 D — 관리자 HTML 구조 에디터

- 상태: 로컬 구현 및 전체 검증 완료
- 이슈: #34
- 변경: 리치 텍스트 에디터 라이브러리 없이 `FormDocument` 모델(`src/lib/forms/document.ts`)과 `parseFormDocument`/`buildFormHtml`을 추가. 왼쪽 패널에서 입력 항목을 추가·삭제·순서·타입·필수·선택지로 구조 편집하고, 오른쪽 격리 미리보기 안에서 제목·안내 문구·제출 버튼 문구를 `contenteditable`로 직접 고침(`buildEditorTemplate`, 일회성 토큰 브리지, `allow-same-origin` 미부여). HTML 직접 입력 탭을 추가하고 기존 파일 업로드 경로는 그대로 유지. 템플릿 상세에는 "복제해 새 템플릿 만들기"만 제공해 기존 템플릿 수정을 막음
- 실행 중 발견·수정: 등록 HTML 계약이 `meta` 태그를 금지해 에디터 골격에서 제거함. 제출 버튼(`button`)은 스페이스 키를 텍스트 입력이 아닌 클릭 활성화로 가로채 문구 편집이 씹히므로 버튼 내부에 삽입한 `span`에 `contenteditable`을 부여하도록 수정. 미리보기 iframe에서 제출 버튼 클릭이 실제 폼 제출을 트리거해 부작용이 있어 에디터 브리지에 `submit` 이벤트 차단을 추가. shadcn `Tabs`의 `data-horizontal:flex-col` variant가 매칭되지 않아 좁은 화면에서 탭 목록이 전체 문서 높이로 늘어나 콘텐츠를 밀어내는 회귀를 발견해 `TemplateEditor`에 `flex-col`을 명시적으로 지정해 수정
- 문서: 에디터 도입 배경과 비파괴 등록 정책을 ADR-008로 기록
- 검증: lint, typecheck, 단위 테스트 58건(왕복 동등성, 생성 HTML의 `validateFormHtml` 통과, 지원하지 않는 타입 거부, 브리지 스크립트 내용 포함), 새 로컬 DB 초기화 후 통합 테스트 8건, 데스크톱·모바일 E2E 18건(구조 편집기만으로 템플릿을 만들어 공개 폼 신청까지 완료, 에디터 iframe의 동일 출처 미부여와 토큰 불일치 메시지 무시 확인 포함), 프로덕션 build 통과
