...
import { normalizeFormFields, type FormFieldSchema } from "@/lib/forms/types"
...
type FormField = FormFieldSchema
...
export function CampaignInsights({ id, inputSchema: rawInputSchema }: { id: string; inputSchema: FormField[] }) {
  const inputSchema = normalizeFormFields(rawInputSchema)
  ...
  return <div className="flex flex-col gap-6">
    ...
    <Dialog open={!!selected} onOpenChange={open => { if (!open) setSelected(null) }}><DialogContent className="max-h-[85vh] sm:max-w-2xl"><DialogHeader><DialogTitle>신청 입력값</DialogTitle><DialogDescription>{selected ? `${koreaTime(selected.submitted_at)} · ${LABELS[selected.channel]}` : "신청 상세"}</DialogDescription></DialogHeader><dl className="grid max-h-[65vh] gap-3 overflow-y-auto pr-1 sm:grid-cols-2">{selected ? Object.entries(selected.values).map(([key, value]) => { const field = inputSchema.find((item) => item.name === key); return <div key={key} className="min-w-0 rounded-lg border bg-muted/30 p-4"><dt><span className="font-medium">{field?.label ?? key}</span>{field?.required ? <Badge variant="secondary" className="ml-2">필수</Badge> : null}<span className="mt-1 block text-xs text-muted-foreground">필드명: {key}</span></dt><dd className="mt-3 break-words text-base">{formatSubmissionValue(field, value)}</dd></div> }) : null}</dl></DialogContent></Dialog>
  </div>
}
