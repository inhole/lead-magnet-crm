...
type Campaign = { id: string; name: string; created_at: string; published_at: string | null; campaign_forms: Array<{ public_id: string; title: string }> }
...
export function CampaignList() {
  ...
  return <div className="overflow-hidden rounded-2xl border bg-card/55">{campaigns.map((campaign) => <Link href={`/campaigns/${campaign.id}`} key={campaign.id} className="flex items-center justify-between gap-4 border-b px-5 py-5 last:border-b-0 hover:bg-card"><div className="flex min-w-0 flex-col gap-1"><span className="truncate font-semibold">{campaign.name}</span><span className="truncate text-sm text-muted-foreground">{campaign.campaign_forms[0]?.title}</span></div><div className="flex shrink-0 flex-col items-end gap-1 sm:flex-row sm:items-center sm:gap-3"><Badge variant={campaign.published_at ? "default" : "secondary"}>{campaign.published_at ? "공개" : "미공개"}</Badge><time className="text-xs text-muted-foreground sm:text-sm" dateTime={campaign.created_at}>{koreaTime(campaign.created_at)}</time></div></Link>)}</div>
}
