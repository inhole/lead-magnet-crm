...
export function TemplateDetail({ templateId }: { templateId: string }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]"><Card><CardHeader><CardTitle>{template.name}</CardTitle><CardDescription><time dateTime={template.created_at}>{new Intl.DateTimeFormat("ko-KR", { timeZone: "Asia/Seoul", dateStyle: "medium", timeStyle: "short" }).format(new Date(template.created_at))}</time></CardDescription></CardHeader><CardContent className="flex flex-col gap-3">{template.input_schema.map((field) => <div key={field.name} className="flex items-center justify-between gap-3 text-sm"><span className="truncate font-medium">{field.label}</span><Badge variant="secondary">{field.type}{field.required ? " · 필수" : ""}</Badge></div>)}</CardContent></Card><Card><CardHeader><CardTitle>폼 미리보기</CardTitle><CardDescription>등록된 HTML을 실행 권한 없는 sandbox에서 표시합니다.</CardDescription></CardHeader><CardContent><iframe title={`${template.name} 미리보기`} sandbox="" srcDoc={preview} className="min-h-[680px] w-full border-0 bg-white" /></CardContent></Card></div>
}
