...
async function respond(id: string, procedure: "get_campaign_links" | "ensure_campaign_links") {
  ...
  const { data, error } = await supabase.rpc(procedure, { p_campaign_id: id })
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) { return respond((await params).id, "get_campaign_links") }
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) { return respond((await params).id, "ensure_campaign_links") }
