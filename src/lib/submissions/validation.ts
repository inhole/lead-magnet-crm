...
export function validateSubmissionValues(input: unknown, fields: FormFieldSchema[]) {
  ...
  for (const field of fields) {
    ...
    if (field.type === "checkbox") {
      const value = typeof raw === "boolean" ? raw : Array.isArray(raw) && raw.every((item) => typeof item === "string") ? raw : false
      if (field.required && (value === false || (Array.isArray(value) && !value.length))) fieldErrors[field.name] = "필수 항목입니다."
    ...
    else if ((field.type === "select" || field.type === "radio") && value && !field.options?.includes(value)) fieldErrors[field.name] = "목록에서 값을 선택하세요."
    else values[field.name] = value
  }
