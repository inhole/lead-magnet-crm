...
export function buildInteractiveTemplate(html: string, copy: FormCopy, bridgeToken: string) {
  ...
  const bridgeScript = `(()=>{const token=${JSON.stringify(bridgeToken)};const sendHeight=()=>{const height=Math.ceil(Math.max(document.body?.scrollHeight||0,document.documentElement.scrollHeight));parent.postMessage({channel:"lead-magnet-form",token,type:"resize",height},"*")};document.documentElement.dataset.formBridge="ready";document.documentElement.style.overflow="hidden";document.addEventListener("submit",event=>{event.preventDefault();const form=event.target;if(!(form instanceof HTMLFormElement))return;const entries=Array.from(new FormData(form).entries(),([name,value])=>[name,String(value)]);parent.postMessage({channel:"lead-magnet-form",token,type:"submit",entries},"*")},true);new ResizeObserver(sendHeight).observe(document.documentElement);addEventListener("load",sendHeight);sendHeight();addEventListener("message",event=>{const data=event.data;if(event.source!==parent||!data||data.channel!=="lead-magnet-form"||data.token!==token||data.type!=="pending")return;const button=document.querySelector("[data-form-submit]");if(button)button.disabled=Boolean(data.value)})})();`
